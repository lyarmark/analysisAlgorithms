...
public class CRT {
	/*
	 * Written By: Gregory Owen Date: 10/10/11 Finds a single congruence
	 * equivalent to multiple given congruences (assuming that one exists) via
	...
	public static int leastPosEquiv(int a, int m) {
		...
		// if 0 <= a < m, then a is the least positive integer equivalent to
		// a mod m
		if (a >= 0 && a < m)
			return a;

		// for negative a, find the least negative integer equivalent to a
		// mod m
		// then add m
		if (a < 0)
